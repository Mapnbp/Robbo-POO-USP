        ...
        try {
            ...
            if (dtde.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
                ...
                if (!files.isEmpty()) {
                    ...
                    // Captura a posição do mouse no momento da soltura
                    this.x = dtde.getLocation().x;
                    this.y = dtde.getLocation().y;

                    this.arquivoSolto = true; // Sinaliza que um arquivo foi solto com sucesso
                    logger.log(Level.INFO, "Arquivo solto: " + this.caminhoArquivoSolto + " em (" + x + "," + y + ")");
                }
            } else {
                dtde.rejectDrop(); // Rejeita o drop se o tipo de dado não for suportado
                this.arquivoSolto = false;
                this.caminhoArquivoSolto = null;
                logger.warning("Tipo de dado arrastado não suportado.");
            }
        } catch (Exception ex) {
            logger.log(Level.SEVERE, "Erro ao processar o arquivo arrastado e solto.", ex);
            this.arquivoSolto = false;
            this.caminhoArquivoSolto = null;
        } finally {
